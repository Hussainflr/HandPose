...
from upe.jpu import JPU as jpu
...
class UnifiedNetwork(nn.Module):
    ...
    def load_conv_bn(self, buf, start, conv_model, bn_model, skip=0):
        ...
        print(data.shape)
        print(conv_model.weight.shape[0], conv_model.weight.shape[1], conv_model.weight.shape[2],
        ...
        if skip < 19:
            ...
            conv_model.weight.data.copy_(
                torch.reshape(torch.from_numpy(buf[start:start + num_w]), (conv_model.weight.data.shape[0],)))
    ...
    def forward(self, x):
        ...
        hf = hf.view(-1, 2 * (self.hand_vector_size + self.num_objects), self.depth_discretization, target_height,
                     target_width)
        print(" HF", hf.shape)
