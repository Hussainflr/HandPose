...
class UnifiedNetwork(nn.Module):
    ...
    # start = self.load_conv_bn(buf, start, self.conv19, self.bn19, True)
    # start = self.load_conv_bn(buf, start, self.conv20, self.bn20, True)
    # start = self.load_conv_bn(buf, start, self.conv21, self.bn21, True)
    # start = self.load_conv_bn(buf, start, self.conv22, self.bn22, True)
    # start = self.load_conv_bn(buf, start, self.conv23, self.bn23, True)
    # print(start)

    def load_conv_bn(self, buf, start, conv_model, bn_model, skip=False):
    ...
    def total_loss(self, pred, true):
       ...
       #MC start
        ...
        total_conf_loss = self.conf_loss(pred_hand_conf, pred_hand_pose, true_hand_pose, hand_mask) + self.conf_loss(
            pred_object_conf, pred_object_pose, true_object_pose, object_mask)
        total_action_loss = self.action_loss(pred_action_prob, true_action_prob, hand_mask)
        #print("probabilities", pred_object_prob[1], true_object_prob[1])
        total_object_loss = self.object_loss(pred_object_prob, true_object_prob, object_mask)
       ...
       #MC End
        ...
        total_loss = total_pose_loss + total_conf_loss - total_object_loss - total_action_loss 
    ...
    def conf_loss(self, pred_conf, pred, true, mask):
        ...
        pixel_conf = torch.exp(parameters.sharpness * (1 - pixel_distance / parameters.pixel_threshold)) / torch.exp(
            parameters.sharpness * (1 - torch.zeros(pixel_distance.size()).cuda(4)))
        depth_conf = torch.exp(parameters.sharpness * (1 - depth_distance / parameters.depth_threshold)) / torch.exp(
            parameters.sharpness * (1 - torch.zeros(depth_distance.size()).cuda(4)))
