...
class UnifiedNetwork(nn.Module):
    ...
    def __init__(self, n_class=21, pretrained=True):
        ...
        self.yoloweights= '../weights/darknet19.weights'
        ...
        self.jpu = JPU([256, 512, 1024], width=512, norm_layer=norm_layer, up_kwargs=up_kwargs)
        ...
        self.conv19 = nn.Conv2d(2048, 1024, 1, stride=1)
        self.bn19 = nn.BatchNorm2d(1024)
        self.relu19 = nn.LeakyReLU(0.1, inplace=True)

        self.jpupool= nn.MaxPool2d(2, stride=2, ceil_mode=True)
        #
        #
        self.conv20 = nn.Conv2d(1024, 720, 3, stride=1, padding=1)
        self.bn20 = nn.BatchNorm2d(720)
        ...
        # self.reorg = Reorg()
        #
        # self.conv22 = nn.Conv2d(1280, 1024, 3, stride=1)
    ...
    def initialize_weights(self, weightfile='../weights/darknet19.weights'):
        ...
        start = 0
        #print("Befor",torch.sum(self.conv1.weight))
        start = self.load_conv_bn(buf, start, self.conv1, self.bn1)
        #print("after", torch.sum(self.conv1.weight))
        #print("Befor",torch.sum(self.conv2.weight))
        start = self.load_conv_bn(buf, start, self.conv2, self.bn2)
        #print("after", torch.sum(self.conv2.weight))

        start = self.load_conv_bn(buf, start, self.conv3, self.bn3)

        start = self.load_conv_bn(buf, start, self.conv4, self.bn4)

        start = self.load_conv_bn(buf, start, self.conv5, self.bn5)
        start = self.load_conv_bn(buf, start, self.conv6, self.bn6)
        start = self.load_conv_bn(buf, start, self.conv7, self.bn7)
        start = self.load_conv_bn(buf, start, self.conv8, self.bn8)
        start = self.load_conv_bn(buf, start, self.conv9, self.bn9)
        start = self.load_conv_bn(buf, start, self.conv10, self.bn10)
        start = self.load_conv_bn(buf, start, self.conv11, self.bn11)
        start = self.load_conv_bn(buf, start, self.conv12, self.bn12)
        start = self.load_conv_bn(buf, start, self.conv13, self.bn13)
        start = self.load_conv_bn(buf, start, self.conv14, self.bn14)
        start = self.load_conv_bn(buf, start, self.conv15, self.bn15)
        start = self.load_conv_bn(buf, start, self.conv16, self.bn16)
        start = self.load_conv_bn(buf, start, self.conv17, self.bn17)
        # start = self.load_conv_bn(buf, start, self.conv18, self.bn18)
        # start = self.load_conv_bn(buf, start, self.conv19, self.bn19, skip=19)
        # start = self.load_conv_bn(buf, start, self.conv20, self.bn20, skip=20)
        # start = self.load_conv_bn(buf, start, self.conv21, self.bn21, skip=21)
        # start = self.load_conv_bn(buf, start, self.conv22, self.bn22, skip=22)
        # start = self.load_conv_bn(buf, start, self.conv23, self.bn23, skip=23)
        #print(start)


    def load_conv_bn(self, buf, start, conv_model, bn_model, skip=0):
        ...
        if skip < 19:
            ...
            conv_model.weight.data.copy_(torch.reshape(torch.from_numpy(buf[start:start + num_w]),(conv_model.weight.data.shape[0], )))
        ...
        #print("At load function", torch.sum(conv_model.weight))
        start = start + num_w
        return start
    ...
    def forward(self, x):
        ...
        c1 = self.relu7(self.bn7(self.conv7(h)))

        #print("Layer  10 :",h.shape )
        h = self.relu8(self.bn8(self.conv8(c1)))
        ...
        c2 = self.relu12(self.bn12(self.conv12(h)))
        #print("Layer 16 :", h.shape)
        h16 = self.relu13(self.bn13(self.conv13(c2)))
        ...
        #c4 is conv17
        c3 = self.relu17(self.bn17(self.conv17(h)))


        c4 = self.relu18(self.bn18(self.conv18(c3)))

        #print("Before C shapes", c1.shape, c2.shape, c3.shape, c4.shape)
        c1, c2, c3, c4 = self.jpu(c1, c2, c3, c4)

        #print("after C shapes", c1.shape, c2.shape, c3.shape, c4.shape)

        hf = self.relu19(self.bn19(self.conv19(c4)))

        hf = self.jpupool(hf)

        hf = self.relu20(self.bn20(self.conv20(hf)))
        ...
        return pred_hand_pose, pred_hand_conf, pred_object_pose, pred_object_prob, pred_object_conf
    ...
    def total_loss(self, pred, true):
        ...
        pred_hand_pose, pred_hand_conf, pred_object_pose, pred_object_prob, pred_object_conf = pred
        true_hand_pose, true_hand_prob, hand_mask, true_object_pose, true_object_prob, object_mask = true
       ...
       #MC End
        ...
        total_loss = total_pose_loss + total_conf_loss - total_object_loss
    ...
    def conf_loss(self, pred_conf, pred, true, mask):
        ...
        #print(type(pixel_distance_mask)
        ...
        true_conf = torch.zeros(pred_conf.size()).cuda(1)
        squared_conf_error = torch.mul(pred_conf - true_conf, pred_conf - true_conf)
